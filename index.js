AFRAME.registerComponent("gaussian_splatting", {
	...
	tick: function (time, timeDelta) {
		...
		if (this.textureReady && this.loadedVertexCount > 0) {
			const scaleThreshold = 5; // Define your threshold for scale here
			const matrices = new Float32Array(this.centerAndScaleData.buffer);
			let validIndexes = [];

			for (let i = 0; i < this.loadedVertexCount; i++) {
				const scale = matrices[i * 4 + 3];
